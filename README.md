...
VULFI has some sample examples written in C and C++ which can be located at <vulfi dir>/examples. You can look at the "common.mk" files inside "src" and "run" subdirectories to understand how VULFI can be used for fault injection. You can compile and run all examples in one go by running below commands.

```
cd <vulfi dir>/examples
make VULFI_SRC_DIR=<vulfi dir> VULFI_INSTALL_DIR=/usr/local/lib
```
...
```
clang++ <vulfi dir>/runtime/Corrupt.C -emit-llvm -c -o Corrupt.bc
```
